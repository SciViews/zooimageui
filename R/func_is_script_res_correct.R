...
is_script_good_results <- function(results_folder_path, selected_script) {
  ...
  if ( selected_script == "[NONE].R" || !grepl(".R", selected_script)) {
    res <- FALSE
    attr(res, "message") <- "Error : Selected Configuration isn't an R script"
    return(res)
  } else {
    ...
    if (!exists("get_results")) {
      res <- FALSE
      attr(res, "message") <- "Error : Selected Configuration doesn't return a function called get_results"
      return(res)
      ...
      res <- FALSE
      attr(res, "message") <- "Error : Selected Configuration doesn't have a data or data_folder_path or Classif or zidb_files argument"
      return(res)
